...
同级：1.png或者./1.png --点/是当前目录 <br/> 下级：22/1.png <br/> 上级：../1.png
--点点/是上级目录，点点/点点/是上上级目录
...
<datalist id="phones">
  <option value="小米11"></option>
  <option value="小米10S"></option>
  <option value="小米笔记本"></option>
  <option value="小米手机"></option>
  <option value="黑鲨4"></option>
...
- id 属性
  - 作用：用于唯一标识页面中的这个文本域元素，是 HTML 元素的通用属性，并非
    `<textarea>`独有。
  - 常见用途：
  1. 配合 CSS 选择器精准样式化该文本域（如 `#feedback { width: 300px; }`）。
  2. 配合 `JavaScript` 获取 / 操作这个元素（如
     `document.getElementById("feedback")`）。
  3. 配合
     `<label>`标签实现 “点击标签聚焦文本域” 的无障碍功能（`<label for="feedback">`留言：`</label><textarea id="feedback"></textarea>`）。
